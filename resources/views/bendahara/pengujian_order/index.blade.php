                            ...
                            <table id="basic-datatables" class="display table table-striped table-hover" >
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @endif
                                        <a href="{{ route('bendahara.pengujian.showBuktiPembayaran', $p->id) }}" class="btn btn-primary btn-xs mb-2">Lihat Bukti Pembayaran</a>
                                        @endif
                                        </td>
                                        <td>@currency($p->total_harga)</td>
                                        <td>
                                            <a href="{{ route('bendahara.pengujian.detailOrder', $p->id) }}" class="btn btn-xs btn-info shadow-sm my-1">
                                            ...
                                            <a href="{{ route('bendahara.pengujian.cetakInvoice', $p->id) }}" target="_blank"><i class="btn btn-xs my-1 btn-primary shadow-sm">Cetak Invoice</i></a> 
                                            <a href="{{ route('bendahara.pengujian.cetakSkr', $p->id) }}" target="_blank"><i class="btn btn-xs btn-primary my-1 shadow-sm">Cetak SKR</i></a> 
                                            @endif
                                            @if ($p->id_status_pengujian >= 5)
                                            <a href="{{ route('bendahara.pengujian.cetakTbp', $p->id) }}" target="_blank"><i class="btn btn-xs btn-primary my-1 shadow-sm">Cetak TBP</i></a> 
                                            @endif
...
<div class="modal fade" id="status" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{ route('bendahara.pengujian.gantiStatus') }}" method="POST">
                @csrf
                @method('PUT')
