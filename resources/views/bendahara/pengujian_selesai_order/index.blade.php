                            ...
                            <table id="basic-datatables" class="display table table-striped table-hover" >
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @endif
                                        <a href="{{ route('bendahara.pengujianSelesai.showBuktiPembayaran', $p->id) }}" class="btn btn-primary btn-xs mb-2">Lihat Bukti Pembayaran</a>
                                        @endif
                                        </td>
                                        <td>@currency($p->total_harga)</td>
                                        <td>
                                            <a href="{{ route('bendahara.pengujianSelesai.detailOrder', $p->id) }}" class="btn btn-xs btn-info shadow-sm my-1">
                                                <i class="fas fa-pencil fa-sm text-white-50"></i>Detail order
                                            </a>
                                            @if ($p->id_status_pengujian >= 4)
                                            <a href="{{ route('bendahara.pengujianSelesai.cetakInvoice', $p->id) }}" target="_blank"><i class="btn btn-xs my-1 btn-primary shadow-sm">Lihat  Invoice</i></a> 
                                            <a href="{{ route('bendahara.pengujianSelesai.cetakSkr', $p->id) }}" target="_blank"><i class="btn btn-xs my-1 btn-primary shadow-sm">Cetak SKR</i></a> 
                                            @endif
                                            @if ($p->id_status_pengujian >= 5)
                                            <a href="{{ route('bendahara.pengujianSelesai.cetakTbp', $p->id) }}" target="_blank"><i class="btn btn-xs btn-primary my-1 shadow-sm">Cetak TBP</i></a> 
                                            @endif
